...
import { Search, Package, Clock, CheckCircle, XCircle, AlertTriangle, ExternalLink, RefreshCw, Eye, CreditCard, Timer, Filter } from 'lucide-react'
...
export default function HistoryPage() {
  ...
  const fetchOrders = async () => {
    try {
      const response = await fetch("/api/orders?type=premium_account")
      if (response.ok) {
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">Please login to view your order history.</p>
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-600">Kelola dan lihat semua pembelian akun premium Anda</p>
        </div>
              ...
              <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                    ...
                    <div className="mt-4 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-orange-800">
                          ⚠️ Order akan expired dalam {order.minutes_until_expiry} menit! Selesaikan pembayaran sekarang atau batalkan untuk melepas stok.
                        </span>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                      ...
                      {order.auto_cancelled_at && (
                        <p className="text-xs text-red-600 mt-1">
                          Auto-cancelled pada {formatDate(order.auto_cancelled_at)}
                        </p>
                      )}
                    </div>
                  )}
                ...
                <div className="flex flex-col gap-2 lg:w-48">
                  ...
                  {order.status === "pending" && order.minutes_until_expiry <= 0 && (
                    <p className="text-xs text-orange-600 text-center">Order expired - akan dibatalkan otomatis</p>
                  )}
                  {order.status === "completed" && (
                    <p className="text-xs text-green-600 text-center">✅ Order selesai</p>
                  )}
                  {order.status === "cancelled" && (
                    <p className="text-xs text-red-600 text-center">❌ Order dibatalkan</p>
                  )}
                </div>
              </div>
            </CardContent>
            ...
            <div className="p-6">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-medium text-gray-600">Nomor Order</label>
                ...
                {/* Items */}
                <div>
                  <label className="text-sm font-medium text-gray-600 mb-3 block">Item Order</label>
                  <div className="space-y-3">
